...
describe('备份版本号与删除（0.35.2）', () => {
  it('list 读取备份内 package.json 的版本号', async () => {
    const root = await tmpRoot()
    const plugins = path.join(root, 'plugins')
    await fs.mkdir(plugins, { recursive: true })
    const backups = new PluginBackups(path.join(root, 'plugin-backups'))
    const dir = path.join(plugins, 'demo')
    await fs.mkdir(dir, { recursive: true })
    await fs.writeFile(path.join(dir, 'package.json'), JSON.stringify({ name: 'demo', version: '1.2.3' }))
    await fs.writeFile(path.join(dir, 'main.js'), 'x')
    await backups.snapshot(dir, 'demo', 'overwrite')
    const list = await backups.list('demo')
  ...
  it('remove 删除指定备份（带 id 校验）', async () => {
    const root = await tmpRoot()
    const plugins = path.join(root, 'plugins')
    await fs.mkdir(plugins, { recursive: true })
    const backups = new PluginBackups(path.join(root, 'plugin-backups'))
    ...
    await sleep(5)
    await fs.writeFile(path.join(dir, 'main.js'), 'v2')
    const b2 = await backups.snapshot(dir, 'demo', 'overwrite')
    expect((await backups.list('demo')).length).toBe(2)
    await backups.remove('demo', b2!.id)
    const list = await backups.list('demo')
    expect(list.length).toBe(1)
    expect(list[0]!.id).not.toBe(b2!.id)
    // 非法 id 抛错
