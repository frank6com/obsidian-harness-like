...
Tools (`ctx.toolsCompat.register`), commands, ItemView panels, ribbon icons, status bar items, settings tabs, UI translations (`ctx.dshI18n`) — signatures in the [Services Reference](/en/dev/services). Full walkthrough: [Your First Plugin](/en/dev/hello-world).
