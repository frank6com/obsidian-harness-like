...
> 子插件注册的命令会显示为 `Harness Like: 命令（子插件id）`，同样在命令面板使用，见[用户插件体系](/zh/guide/plugins)。

## 下一步

- [对话面板详解](/zh/guide/chat)（含会话导出）
- [智能体与模型](/zh/guide/agents-models)（三种模式的用途）
- [在对话中描述需求](/zh/guide/speak-to-agent)（Obsidian 区域术语与对话表述）
