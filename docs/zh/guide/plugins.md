...
用户插件是运行在 Obsidian 内的 **Cordis 插件**（由 Harness Like 加载），用于扩展命令、工具、面板等能力——可用扩展点一览见[能力清单（扩展点映射）](/zh/development/capabilities)。
...
- 首段固定为 `hl`（Harness Like 命名空间，不会与 mermaid 等原生语言或其他插件冲突），其后第一个参数是子插件 id，后面可接 `:<类型>`；
- **类型可省略**：子插件注册了名为 `default` 的类型、或只注册了一个类型时，写 `` ```hl tasks `` 即可；注册了多个类型则必须显式指定，笔记会提示可选类型；
- **参数可选且顺序无关**：`k:v`、`k=v`、`k:"含 空格 的值"`、`--flag`、`--k=v`、裸词，分别交给渲染器的 `meta.params` / `meta.flags` / `meta.positional`；
- 插件未运行时块显示"未运行"占位，重新加载即恢复；**块内容为空时显示"内容为空"提示框**；
- 在插件管理器 → 详情中可为该插件设置【插件别名】（如 `tasks` → `t`），之后 `` ```hl t:board `` 同样生效，别名也可省略类型；
- 旧写法 `` ```hl:<子插件id>:<类型> ``（冒号式语言串）**已不支持**，笔记中会提示新写法。
