...
本栏目面向 **Harness Like 插件的开发者/贡献者**（想完善本插件本身），与面向使用者的[插件开发](/zh/dev/hello-world)（在 Obsidian 里写用户插件）不同。
...
详细流程见[开发流程](/zh/development/workflow)，版本规则见[版本号规范](/zh/development/versioning)，发布见[发布与审核](/zh/development/release)，约定见[架构约定](/zh/development/conventions)。
